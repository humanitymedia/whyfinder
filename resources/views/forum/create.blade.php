<x-public-layout>
    <x-slot name="title">New Thread — {{ $course->title }}</x-slot>
    ...
    <section class="bg-brand-brown text-white py-12">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <nav class="flex items-center gap-2 text-sm mb-4">
                ...
                <span class="text-gray-500">/</span>
                <span class="text-gray-300">New Thread</span>
            </nav>
            <h1 class="text-2xl md:text-3xl font-bold">Start a New Discussion</h1>
        </div>
    </section>

    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <a href="{{ route('forum.index', $course->slug) }}" class="inline-flex items-center gap-1 text-sm text-brand-gray hover:text-brand-dark transition-colors mb-6">
            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M10.5 19.5L3 12m0 0l7.5-7.5M3 12h18" /></svg>
            Back to forum
        </a>

        <div class="bg-white rounded-xl border border-brand-gray-light p-6 md:p-8">
            <form method="POST" action="{{ route('forum.store', $course->slug) }}" class="space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <textarea name="body" id="body" rows="8" required
                              class="w-full rounded-lg border-gray-300 shadow-sm focus:border-brand-brown focus:ring-brand-brown text-sm"
                              placeholder="Describe your question or share your thoughts...">{{ old('body') }}</textarea>
                    @error('body')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex items-center gap-3 pt-2">
                    ...
                    <a href="{{ route('forum.index', $course->slug) }}" class="text-sm text-brand-gray hover:text-brand-dark transition-colors">Cancel</a>
                </div>
            </form>
        </div>
    </div>
